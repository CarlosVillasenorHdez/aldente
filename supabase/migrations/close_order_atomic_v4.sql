-- ══════════════════════════════════════════════════════════════════════════════
-- Aldente ERP — close_order v4
-- Fix: tenant_id es TEXT en las tablas, no UUID → cambiar parámetro a TEXT
-- Ejecutar en: Supabase → SQL Editor
-- ══════════════════════════════════════════════════════════════════════════════

-- Eliminar todas las versiones anteriores (firmas diferentes)
...
DROP FUNCTION IF EXISTS close_order(TEXT, TEXT,  NUMERIC, NUMERIC, NUMERIC, NUMERIC, TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER, NUMERIC, TEXT[]);
DROP FUNCTION IF EXISTS close_order(TEXT, TEXT,  NUMERIC, NUMERIC, NUMERIC, NUMERIC, TEXT, TEXT, TEXT, TEXT, UUID,  INTEGER, NUMERIC, TEXT[]);

CREATE OR REPLACE FUNCTION close_order(
  p_order_id            TEXT,
  p_tenant_id           TEXT,      -- TEXT, no UUID — el schema usa TEXT para tenant_id
  p_subtotal            NUMERIC,
  p_iva                 NUMERIC,
  p_discount            NUMERIC,
  p_total               NUMERIC,
  ...
  p_branch              TEXT,
  p_opened_at           TEXT,
  p_loyalty_customer_id TEXT,      -- TEXT también (puede ser NULL)
...
BEGIN
  ...
  -- Cast seguro TEXT → payment_method enum
  BEGIN
