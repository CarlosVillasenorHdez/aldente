-- ══════════════════════════════════════════════════════════════════════════════
-- Aldente ERP — close_order v6
-- Causa raíz: tenant_id es UUID en DB (migración multi_tenant.sql),
-- pero la función recibía TEXT sin cast.
-- Solución: parámetros siguen siendo TEXT (el JS pasa strings),
-- pero internamente se castea a UUID donde la columna lo requiere.
-- Ejecutar en: Supabase → SQL Editor
-- ══════════════════════════════════════════════════════════════════════════════

-- 1. Borrar TODAS las versiones existentes sin importar firma
DO $$
DECLARE r RECORD;
BEGIN
  FOR r IN
    SELECT oid::regprocedure::text AS sig FROM pg_proc
    WHERE proname = 'close_order'
...
-- 2. Crear versión correcta
CREATE FUNCTION close_order(
  p_order_id            TEXT,
  p_tenant_id           TEXT,      -- recibe TEXT desde JS, se castea a UUID internamente
  p_subtotal            NUMERIC,
  p_iva                 NUMERIC,
  p_discount            NUMERIC,
  p_total               NUMERIC,
  ...
  p_branch              TEXT,
  p_opened_at           TEXT,
  p_loyalty_customer_id TEXT,      -- puede ser NULL
...
DECLARE
  ...
  v_tenant_uuid   UUID;           -- tenant_id casteado a UUID
  v_loyalty_uuid  UUID;           -- loyalty_customer_id casteado a UUID (nullable)
...
BEGIN
  ...
  -- Cast TEXT → UUID para loyalty_customer_id (nullable)
  IF p_loyalty_customer_id IS NOT NULL AND p_loyalty_customer_id != '' THEN
    BEGIN
      v_loyalty_uuid := p_loyalty_customer_id::UUID;
    EXCEPTION WHEN invalid_text_representation THEN
      v_loyalty_uuid := NULL;
    END;
  ELSE
  ...
  -- Cast TEXT → payment_method enum con fallback
  BEGIN
  ...
  SET
    ...
    updated_at     = v_now
  WHERE id        = p_order_id
    AND tenant_id = v_tenant_uuid;   -- UUID ✓
  ...
  BEGIN
    UPDATE orders SET loyalty_customer_id = v_loyalty_uuid
  ...
  FOR v_rec IN
    ...
    JOIN dish_recipes dr ON dr.dish_id   = oi.dish_id
                        AND dr.tenant_id = v_tenant_uuid
    JOIN ingredients  i  ON i.id         = dr.ingredient_id
                        AND i.tenant_id  = v_tenant_uuid
    WHERE oi.order_id   = p_order_id
      AND oi.tenant_id  = v_tenant_uuid
  ...
  LOOP
    ...
    INSERT INTO stock_movements (
      ...
      v_tenant_uuid, v_rec.ingredient_id, 'salida',
  ...
  UPDATE orders
  SET cost_actual = v_cost_actual, margin_actual = v_margin_actual, margin_pct = v_margin_pct
  WHERE id = p_order_id AND tenant_id = v_tenant_uuid;

  -- ── PASO 4: Liberar mesas ──────────────────────────────────────────────────
  UPDATE restaurant_tables
  SET status='libre', current_order_id=NULL, waiter=NULL,
  ...
  IF v_loyalty_uuid IS NOT NULL AND p_loyalty_points > 0 THEN
    ...
    SET points      = COALESCE(points,0) + p_loyalty_points,
    ...
    INSERT INTO loyalty_transactions (
      ...
      v_tenant_uuid, v_loyalty_uuid, p_order_id,
      'acumulacion', p_loyalty_points, p_total,
      'Compra — ' || p_order_id, v_now
    );
  END IF;
