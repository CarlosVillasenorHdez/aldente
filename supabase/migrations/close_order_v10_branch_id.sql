-- close_order v10 — agrega p_branch_id para que las órdenes se guarden
-- con branch_id correcto y aparezcan en reportes por sucursal
...
CREATE FUNCTION close_order(
  ...
  p_tip                 NUMERIC,
  p_table_ids           TEXT[],
  p_branch_id           TEXT DEFAULT NULL   -- UUID de sucursal (opcional para compatibilidad)
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- table_ids — filtrar solo UUIDs reales (ignora "takeout-ORD-xxx")
  ...
  -- PASO 1: Cerrar la orden — guardar branch_id para reportes por sucursal
  UPDATE orders SET
    subtotal       = p_subtotal,
    iva            = p_iva,
    discount       = p_discount,
...
-- Parchar la orden existente con branch_id null (si el restaurante solo tiene 1 sucursal)
-- Descomentar y ajustar el UUID si se conoce el branch_id:
-- UPDATE orders SET branch_id = 'UUID-DE-TU-SUCURSAL' WHERE branch_id IS NULL AND tenant_id = '6e915d0a-6ebc-4629-a030-2f5d332581d8';
