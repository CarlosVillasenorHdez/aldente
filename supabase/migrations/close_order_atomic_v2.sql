-- ══════════════════════════════════════════════════════════════════════════════
-- Aldente ERP — Función close_order atómica v2
-- Correcciones:
--   - UPDATE de orders dividido: campos seguros primero, campos opcionales después
--   - restaurant_tables: columnas opcionales en bloque separado
--   - Mejor manejo de excepciones con mensaje claro
...
BEGIN
  ...
  -- ── PASO 1: Cerrar la orden — solo columnas que SIEMPRE existen ────────────
  UPDATE orders
  SET
    subtotal       = p_subtotal,
    iva            = p_iva,
  ...
  -- ── PASO 4: Liberar mesas — columnas seguras primero ──────────────────────
...
EXCEPTION WHEN OTHERS THEN
  -- Mensaje claro con el error real de PostgreSQL
  RAISE EXCEPTION 'close_order[%]: % — Detalle: %',
