-- =============================================================================
-- DEMO MENU SEED — Platillos, ingredientes y recetas para los 3 restaurantes demo
-- Restaurantes: Dummy Demo (Restaurante Principal), Dragon Wok, Barista
-- =============================================================================

DO $$
DECLARE
  -- ── Tenant IDs ─────────────────────────────────────────────────────────────
  tid_demo   UUID := '00000000-0000-0000-0000-000000000001'::uuid;  -- Dummy Demo / Restaurante Principal
  tid_wok    UUID;  -- Dragon Wok
  tid_bar    UUID;  -- Barista
  ...
  w_dish_rol_pri2 UUID := gen_random_uuid();

  -- ══════════════════════════════════════════════════════════════════════════
  -- BARISTA — Ingredientes
  -- ══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- ════════════════════════════════════════════════════════════════════════
  -- DRAGON WOK — Solo si el tenant existe
  -- ════════════════════════════════════════════════════════════════════════
  IF tid_wok IS NOT NULL THEN
    ...
    INSERT INTO public.ingredients (id, name, category, stock, unit, min_stock, cost, supplier, reorder_point, supplier_phone, tenant_id, branch_id) VALUES
      ...
      (w_ing_ace_veg, 'Aceite Vegetal',         'Aceites y Salsas',8,    'lt',  3,    48,  'Abarrotes Don Pepe',   4,   '55 9090 1212', tid_wok, bid_wok),
      (w_ing_vin_arr, 'Vinagre de Arroz',       'Aceites y Salsas',3,    'lt',  1,    70,  'Distribuidora Asia',   2,   '55 4400 5500', tid_wok, bid_wok),
      (w_ing_azu,     'Azúcar',                 'Abarrotes',       5,    'kg',  2,    25,  'Abarrotes Don Pepe',   3,   '55 9090 1212', tid_wok, bid_wok),
    ...
    INSERT INTO public.dishes (id, name, description, price, category, available, emoji, popular, tenant_id, branch_id) VALUES
      ...
      (w_dish_rol_pri2, 'Salsa Agridulce Extra',      'Porción extra de salsa agridulce casera.',                                                                                     20,  'Extras',         true, '🥫', false, tid_wok, bid_wok)
    ON CONFLICT (id) DO NOTHING;
  ...
  -- ════════════════════════════════════════════════════════════════════════
  -- BARISTA — Solo si el tenant existe
  -- ════════════════════════════════════════════════════════════════════════
  IF tid_bar IS NOT NULL THEN
  ...
  RAISE NOTICE 'Demo menu seed completado. Dummy Demo: 21 platillos, 47 ingredientes. Dragon Wok: % platillos. Barista: % platillos.',
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE NOTICE 'Error en demo menu seed: %', SQLERRM;
    RAISE;
END $$;
