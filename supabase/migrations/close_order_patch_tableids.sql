-- Patch: corregir el doble unnest en v_table_uuids
-- Solo reemplaza esa línea — el resto de la función queda igual
CREATE OR REPLACE FUNCTION close_order(
  p_order_id TEXT, p_tenant_id TEXT,
  p_subtotal NUMERIC, p_iva NUMERIC, p_discount NUMERIC, p_total NUMERIC,
  p_pay_method TEXT, p_mesero TEXT, p_branch TEXT, p_opened_at TEXT,
  p_loyalty_customer_id TEXT, p_loyalty_points INTEGER,
  p_tip NUMERIC, p_table_ids TEXT[]
)
...
BEGIN
  ...
  -- Filtrar solo UUIDs válidos (ignora "takeout-ORD-xxx")
  ...
  UPDATE orders SET
    ...
    closed_at=v_now_iso, updated_at=v_now_iso
  ...
  -- PASO 2: Descontar inventario + COGS
  FOR v_rec IN
  ...
  -- PASO 4: Liberar mesas reales (ignora takeout)
  ...
  IF v_loyalty IS NOT NULL AND p_loyalty_points>0 THEN
    ...
    INSERT INTO loyalty_transactions(tenant_id,customer_id,order_id,type,points,amount,notes,created_at)
    VALUES(v_tenant,v_loyalty,p_order_id,'acumulacion',p_loyalty_points,p_total,'Compra — '||p_order_id,v_now);
  END IF;
