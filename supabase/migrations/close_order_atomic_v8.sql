-- close_order v8 — maneja IDs virtuales de takeout (no son UUID)
-- "takeout-ORD-xxx" no es un UUID → se filtra antes del cast
...
BEGIN
  ...
  -- Castear tenant_id TEXT → UUID
  v_tenant := p_tenant_id::UUID;

  -- Filtrar p_table_ids: solo los que son UUID válidos (ignora "takeout-xxx")
  SELECT ARRAY_AGG(t::UUID)
  INTO v_table_uuids
  FROM unnest(p_table_ids) AS t
  WHERE t ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$';
  -- Si ninguno es UUID válido (orden takeout) queda NULL → no se actualiza restaurant_tables
  ...
  -- pay_method TEXT → enum con fallback
  BEGIN
  ...
  -- ── PASO 4: Liberar mesas (solo si hay IDs UUID reales) ───────────────────
  IF v_table_uuids IS NOT NULL AND array_length(v_table_uuids, 1) > 0 THEN
    ...
    EXCEPTION WHEN undefined_column THEN NULL; END;
  END IF;
  -- Si era takeout (sin UUID) no hay mesa física que liberar — está bien
