-- close_order v7 — tipos exactos confirmados por diagnóstico
-- orders.id = TEXT, todo lo demás (tenant_id, ingredient_id, etc.) = UUID
-- restaurant_tables.id = UUID → cast p_table_ids TEXT[] → UUID[]
...
CREATE FUNCTION close_order(
  p_order_id            TEXT,       -- orders.id es TEXT
  p_tenant_id           TEXT,       -- recibe string desde JS, se castea a UUID
  p_subtotal            NUMERIC,
  p_iva                 NUMERIC,
  p_discount            NUMERIC,
  p_total               NUMERIC,
  ...
  p_branch              TEXT,
  p_opened_at           TEXT,
  p_loyalty_customer_id TEXT,       -- nullable, se castea a UUID si viene
  p_loyalty_points      INTEGER,
  p_tip                 NUMERIC,
  p_table_ids           TEXT[]      -- restaurant_tables.id es UUID, se castea abajo
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Castear todos los TEXT → UUID de una vez
  v_tenant      := p_tenant_id::UUID;
  v_table_uuids := ARRAY(SELECT unnest(p_table_ids)::UUID);
  ...
  -- PASO 4: Liberar mesas (restaurant_tables.id = UUID)
