...
export default function MenuGrid({ items, onAddItem, orderItems, selectedTable }: MenuGridProps) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 py-3 bg-[#162d55] border-b flex flex-col gap-2 flex-shrink-0" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
      ...
      {!selectedTable && (
        <div className="mx-4 mt-4 px-4 py-3 rounded-xl flex items-center gap-3 text-sm" style={{ backgroundColor: 'rgba(245,158,11,0.08)', border: '1px solid #fde68a' }}>
        ...
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-48 text-center gap-3">
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center" style={{ backgroundColor: 'rgba(255,255,255,0.06)' }}>
              <UtensilsCrossed size={24} className="text-white/30" />
            </div>
            <div>
              <p className="text-sm font-semibold text-white/70 mb-1">No hay platillos disponibles</p>
              <p className="text-xs text-white/40">Agrega platillos desde el módulo de Menú para verlos aquí.</p>
            </div>
          </div>
        ) : filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-40 text-center">
            <span className="text-3xl mb-2">🍽️</span>
            <p className="text-sm font-semibold text-white/70">No se encontraron platillos</p>
            <p className="text-xs text-white/40 mt-1">Intenta con otra búsqueda o categoría</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-4 gap-3">
            {filtered.map((item) => {
              ...
              const catColors = categoryColors[item.category] || { bg: '#f3f4f6', color: 'rgba(255,255,255,0.75)' };
              ...
              return (
                ...
                >
                  ...
                  <div className="p-3 relative">
                    ...
                    <p className="text-xs text-white/40 leading-tight mb-2 line-clamp-2">{item.description}</p>
                    <div className="flex items-center justify-between">
                      ...
                      {!item.available ? (
                        ...
                        <div className="w-7 h-7 rounded-lg flex items-center justify-center transition-colors" style={{ backgroundColor: 'rgba(255,255,255,0.06)' }}>
