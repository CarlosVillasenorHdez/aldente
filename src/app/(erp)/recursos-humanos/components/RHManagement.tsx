...
type ActiveTab = 'vacaciones' | 'permisos' | 'tiempos_extras' | 'incapacidades' | 'resumen' | 'liquidacion' | 'pago_nomina' | 'asistencia';
...
export default function RHManagement() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      try {
        const incapRes = await supabase.from('rh_incapacidades').select('*, employees(name, role)')
        .eq('tenant_id', getTenantId()).order('created_at', { ascending: false });
  ...
  async function saveIncapacidad() {
    ...
    try {
      ...
      setShowModal(null);
      // Reload
      const res = await supabase.from('rh_incapacidades').select('*, employees(name, role)')
        .eq('tenant_id', getTenantId()).order('created_at', { ascending: false });
