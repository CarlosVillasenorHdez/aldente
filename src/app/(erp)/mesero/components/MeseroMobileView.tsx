                ...
                {[{id:'all',name:'Todas',icon:'🍽️',color:'#b45309'},...sections].map(s=>{
                  ...
                  return (
                    <button key={s.id} onClick={()=>setActiveSection(s.id as any)}
                      style={{flexShrink:0,display:'flex',alignItems:'center',gap:5,padding:'8px 14px',borderRadius:20,border:`1.5px solid ${isActive?s.color+'80':'#e5e7eb'}`,background:isActive?s.color+'12':'#f9fafb',color:isActive?s.color:'#6b7280',fontSize:13,fontWeight:isActive?700:500,cursor:'pointer',whiteSpace:'nowrap'}}>                      <span>{s.icon}</span><span>{s.name}</span>
