...
export default function Sidebar({ collapsed, onToggle }: SidebarProps) {
  ...
  // Branch state managed by BranchContext
    ...
    >
        ...
        <div className="flex items-center gap-2 overflow-hidden">
          ...
          {!collapsed && (
            <div className="flex flex-col overflow-hidden">
              ...
              {activeBranchName ? (
                <span className="text-xs truncate" style={{ color: 'rgba(255,255,255,0.45)', lineHeight: 1.2 }}>{activeBranchName}</span>
              ) : (
      ...
      {!collapsed && canSwitch && (
        <div className="px-3 pt-3 relative">
          ...
          >
            <GitBranch size={13} />
            <span className="truncate flex-1 text-left">{activeBranchName}</span>
            ...
            <div className="absolute left-3 right-3 top-full mt-1 z-50 rounded-lg shadow-xl overflow-hidden"
              ...
              {branches.map(b => (
                <button key={b.id} onClick={() => {
                  ...
                  {b.id === activeBranchId && <span className="ml-auto" style={{ color: '#f59e0b' }}>✓</span>}
                </button>
              ))}
            </div>
          )}
